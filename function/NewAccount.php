...
if(isset($_POST['submit'])){
	...
	if(empty($FormData['domain'])){
		...
		if(mysqli_num_rows($sql)<3){
			...
			'plan' => $FormData['plan'],
      'label' => strip_tags('My Lovely Website For '.strtolower($_POST['domain']))  
			]);
			...
			elseif($Result['status']==1 && strlen($Result['message'])>1){
				$sql = mysqli_query($connect,"INSERT INTO `hosting_account`(`account_username`, `account_password`, `account_key`, `account_domain`, `account_status`, `account_date`, `account_for`, `account_sql`, `account_label`) VALUES ('".$Result['username']."','".$FormData['password']."','".$FormData['username']."','".$Result['domain']."','1','".$Result['date']."','".$ClientInfo['hosting_client_key']."','NULL', '".$FormData['label']."')");
				if($sql){
